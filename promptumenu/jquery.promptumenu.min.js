...
(c=e.top+l+a*d.inertia,c<-1*f.height*(j-1)?c=-1*f.height*(j-1):0<c&&(c=0),d.pages?(k=Math.round(-c/f.height),o.go_to(k+1,"inertia")):b.animate({top:c},Math.abs(a*d.inertia),"inertia")):(c=e.left+h+k*d.inertia,c<-1*f.width*(j-1)?c=-1*f.width*(j-1):0<c&&(c=0),d.pages?(k=Math.round(-c/f.width),o.go_to(k+1,"inertia")):b.animate({left:c},Math.abs(k*d.inertia),"inertia"))})});try{var p,m=[],v=function(a){a.preventDefault();for(var f={time:(new Date).getTime(),x:a.touches[0].pageX,y:a.touches[0].pageY};4<
m.length;)m.shift();"vertical"==d.direction?(g=a.touches[0].pageY-t,b.css("top",p.top+g)):(k=a.touches[0].pageX-u,b.css("left",p.left+k));m.push(f)},w=function(a){a.preventDefault();document.removeEventListener("touchmove",v,!1);document.removeEventListener("touchend",w,!1);var e=m[0],c=m[m.length-1],h=c.time-e.time,a=(c.x-e.x)/h,e=(c.y-e.y)/h;"vertical"==d.direction?(c=p.top+g+e*d.inertia,c<-1*f.height*(j-1)?c=-1*f.height*(j-1):0<c&&(c=0),d.pages?(a=Math.round(-c/f.height),o.go_to(a+1,"inertia")):
b.animate({top:c},Math.abs(e*d.inertia),"inertia")):(c=p.left+k+a*d.inertia,c<-1*f.width*(j-1)?c=-1*f.width*(j-1):0<c&&(c=0),d.pages?(a=Math.round(-c/f.width),o.go_to(a+1,"inertia")):b.animate({left:c},Math.abs(a*d.inertia),"inertia"))};b[0].addEventListener("touchstart",function(a){b.unbind(".promptumenu");a.preventDefault();b.stop(!0,!1);p=b.position();u=a.touches[0].pageX;t=a.touches[0].pageY;k=0;g=0;m=[];document.addEventListener("touchmove",v,!1);document.addEventListener("touchend",w,!1)},!1)}catch(y){}}})}})(jQuery);
jQuery.extend(jQuery.easing,{inertia:function(n,g,d,x,k){return x*((g=g/k-1)*g*g+1)+d}});