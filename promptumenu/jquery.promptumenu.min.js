...
height:"auto",width:"auto",top:0,left:0,margin:0,padding:0});b.children("li").css({display:"block",position:"absolute",margin:0});b.children("li").each(function(){var a=m(this);c.x+=1;if(c.x>f.columns)c.x=1,c.y+=1;if(c.y>f.rows)c.x=1,c.y=1,c.page+=1;a.data("layout",m.extend({},c));"vertical"==f.direction?(a.css({top:Math.round(c.y*j-j/2-a.height()/2+(c.page-1)*e.height),left:Math.round(c.x*g-g/2-a.width()/2)}),a.find("img").bind("load",function(){var b=a.data("layout");a.css({top:Math.round(b.y*j-
...
b.data("ppos",a)))};b[0].addEventListener("touchstart",function(a){b.unbind(".promptumenu");a.preventDefault();b.stop(!0,!1);b.css({"-webkit-transition-duration":"0ms"});p=b.data("ppos");v=a.touches[0].pageX;u=a.touches[0].pageY;k=0;d=0;l=[];document.addEventListener("touchmove",w,!1);document.addEventListener("touchend",t,!1);document.addEventListener("touchcancel",t,!1)},!1)}catch(y){alert(y)}}})}})(jQuery);jQuery.extend(jQuery.easing,{inertia:function(m,d,f,x,k){return x*((d=d/k-1)*d*d+1)+f}});